        ...
        VStack(spacing: 16) {
            ...
            TabView(selection: $viewModel.currentExerciseIndex) {
                ForEach(Array(viewModel.exercises.enumerated()), id: \.offset) { index, exercise in
                    largeExerciseCard(exercise: exercise)
                        .tag(index)
                }
            }
        ...
        VStack(spacing: 20) {
            ...
            ZStack {
                ...
                Image(systemName: exercise.icon)
                    ...
                    .foregroundColor(neonYellow)
                    .symbolEffect(.bounce, value: viewModel.currentExerciseIndex)
            }
            ...
            if viewModel.trainingHistory.isEmpty {
                ...
                VStack(spacing: 8) {
                    ForEach(Array(viewModel.trainingHistory.prefix(5).enumerated()), id: \.offset) { index, record in
                        modernRecordRow(record: record, index: index)
...
struct BounceButtonStyle: ButtonStyle {
    ...
    private func exerciseCard(exercise: StrengthExercise, isSelected: Bool) -> some View {
...
class UpperBodyStrengthViewModel: ObservableObject {
    @Published var exercises: [StrengthExercise] = [
        StrengthExercise(name: "卧推", nameEnglish: "Bench Press", icon: "figure.strengthtraining.traditional", targetSets: 4),
        StrengthExercise(name: "哑铃划船", nameEnglish: "Dumbbell Rows", icon: "figure.arms.open", targetSets: 4)
    ]
    ...
    func logSet() {
        ...
        // 根据组数判断类型
        let setType: SetType
        let currentExerciseSets = trainingHistory.filter { $0.exerciseName == currentExercise.name }.count
        
        if currentExerciseSets == 0 {
            setType = .warmup
        } else if currentExerciseSets == 1 {
            setType = .target
        } else {
            setType = .working
        }
